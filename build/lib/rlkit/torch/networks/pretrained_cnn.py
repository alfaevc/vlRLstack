...
class PretrainedCNN(PyTorchModule):
    ...
    def __init__(
        ...
        # print(f"self.conv_output_flat_size: {self.conv_output_flat_size }")
        if self.output_conv_channels:
            self.last_fc = None
        else:
            fc_input_size = self.conv_output_flat_size
            # used only for injecting input directly into fc layers
            # print(f"fc_input_size {fc_input_size} added_fc_input_size {added_fc_input_size}")
    ...
    def forward(self, input, new_obs_actions, return_last_activations=False):
        # print(input.shape, end="\t")
        input = torch.cat([input, new_obs_actions], dim=-1)
        # if isinstance(return_last_activations, torch.Tensor):
        #     print(return_last_activations.shape)
        # else:
        #     print()
