...
def make_quantile_nerwork(
    ...
    """
    dummy_state = state_space.sample()[None, ...]
    if len(state_space.shape) == 1:
        dummy_state = dummy_state.astype(np.float32)
    network_dict = {}
    params_dict = {}

    if len(state_space.shape) == 3:
        network_dict["feature"] = hk.without_apply_rng(hk.transform(lambda s: DQNBody()(s)))
        dummy_feature = np.zeros((1, 7 * 7 * 64), dtype=np.float32)
    else:
        network_dict["feature"] = hk.without_apply_rng(hk.transform(lambda s: s))
        dummy_feature = dummy_state
    params_dict["feature"] = network_dict["feature"].init(next(rng), dummy_state)

    dummy_cum_p = np.empty((1, num_quantiles), dtype=np.float32)
    network_dict["quantile"] = hk.without_apply_rng(hk.transform(fn))
    params_dict["quantile"] = network_dict["quantile"].init(next(rng), dummy_feature, dummy_cum_p)
    ...
    return network_dict, params_dict, dummy_feature
...
def make_stochastic_latent_variable_model(
    ...
    # dummy input for JIT compilations.
    dummy_state_ = jnp.empty((1, num_sequences, *state_space.shape), dtype=jnp.uint8)
    dummy_action_ = jnp.empty((1, num_sequences, *action_space.shape))
    dummy_action = jnp.empty((1, *action_space.shape))
    dummy_feature = jnp.empty((1, feature_dim))
    dummy_z_ = jnp.empty((1, num_sequences, z1_dim + z2_dim))
    dummy_z1_ = jnp.empty((1, num_sequences, z1_dim))
    dummy_z2_ = jnp.empty((1, num_sequences, z2_dim))
    dummy_z1 = jnp.empty((1, z1_dim))
    dummy_z2 = jnp.empty((1, z2_dim))
    ...
    params_dict["z1_prior_init"] = network_dict["z1_prior_init"].init(next(rng), dummy_action)

    network_dict["z1_prior"] = hk.without_apply_rng(hk.transform(fn_z1_prior))
    params_dict["z1_prior"] = network_dict["z1_prior"].init(next(rng), dummy_z2, dummy_action)

    network_dict["z1_post_init"] = hk.without_apply_rng(hk.transform(lambda x: Gaussian(z1_dim, units_model)(x)))
    params_dict["z1_post_init"] = network_dict["z1_post_init"].init(next(rng), dummy_feature)

    network_dict["z1_post"] = hk.without_apply_rng(hk.transform(fn_z1_post))
    params_dict["z1_post"] = network_dict["z1_post"].init(next(rng), dummy_feature, dummy_z2, dummy_action)

    network_dict["z2_init"] = hk.without_apply_rng(hk.transform(lambda x: Gaussian(z2_dim, units_model)(x)))
    params_dict["z2_init"] = network_dict["z2_init"].init(next(rng), dummy_z1)

    network_dict["z2"] = hk.without_apply_rng(hk.transform(fn_z2))
    params_dict["z2"] = network_dict["z2"].init(next(rng), dummy_z1, dummy_z2, dummy_action)

    network_dict["reward"] = hk.without_apply_rng(hk.transform(fn_reward))
    params_dict["reward"] = network_dict["reward"].init(next(rng), dummy_z_, dummy_action_, dummy_z_)

    network_dict["encoder"] = hk.without_apply_rng(hk.transform(fn_encoder))
    params_dict["encoder"] = network_dict["encoder"].init(next(rng), dummy_state_)

    network_dict["decoder"] = hk.without_apply_rng(hk.transform(fn_decoder))
    params_dict["decoder"] = network_dict["decoder"].init(next(rng), dummy_z1_, dummy_z2_)
