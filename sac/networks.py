...
def make_networks(
  ...
  def _critic_fn(obs, action):
    ...
    #obs = network1(obs)
    #obs = jnp.squeeze(obs, axis=0)
    #print(obs.shape)
    #action = jax.nn.one_hot(action,num_classes=num_dimensions)
    ...
    #print(jnp.concatenate([value1, value2], axis=-1))
  ...
  dummy_action = jnp.stack([dummy_action]*128)
  dummy_action = jnp.stack([dummy_action]*128)
  dummy_obs = utils.zeros_like(spec.observations)
  dummy_obs = jax.tree_leaves(dummy_obs)
  
  #print(dummy_obs[0].shape)
  dummy_obs = jnp.concatenate((dummy_obs[0], dummy_obs[1], dummy_obs[2]),axis=2)
  #print(dummy_obs.shape)
  dummy_action = utils.add_batch_dim(dummy_action)
  dummy_obs = utils.add_batch_dim(dummy_obs)
  dummy_obs = dummy_obs.astype(float)
  #print(dummy_obs.shape)
